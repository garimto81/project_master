...
export async function PATCH(request: NextRequest, { params }: Params) {
  try {
    ...
    const result = await updateIssueState(owner, repoName, issueNumber, 'open', auth.token!)

    if (!result.success) {
      return errorResponse(result.error || `Failed to reopen issue: #${number}`, 400)
    }

    return NextResponse.json({ success: true })
  } catch (error) {
    console.error('Reopen issue API error:', error)
    return errorResponse('Failed to reopen issue', 500, error)
  }
}
